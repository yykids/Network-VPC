...
192.253.0.0/24 | 使用できます。使用可能な範囲を超えました。
...
VPCはサブネットに分けて、小さなネットワークを複数構成できます。ただしサブネットの場合はVPCアドレスの範囲に含まれ、アドレスの長さがそれ以下である必要があります。例えば192.168.0.0/16の場合、192.168.0.0 ~ 192.168.255.255まで、合計65536個のIPアドレスを使用できます。また最も小さなサブネットは30bitで、これより小さく構成することはできません。サブネットもVPC同様、CIDR表記を使用します。

サブネットが作成されると、ゲートウェイIPアドレスは自動的に指定され、これを変更できません。またVPCに含まれたルーティングテーブルに自動的に登録されます。 

> [参考]
サブネットは、インスタンスやロードバランサーなどが該当のサブネットに含まれていない空のサブネットの場合にのみ削除できます。
...
* 1つのインスタンスに2個のサブネットを接続し、2個のFloating IPを接続できます。
この場合、別途設定が必要です。 [Linux Advanced Routing](http://lartc.org/)
...
インスタンスをネットワーク上の他のトラフィックから保護する目的で使用できます。通常、指定したものを許容し、残りは遮断する"ポジティブセキュリティモデル"により動作します。最初にサービスが開始すると、1個の基本セキュリティーグループが生成され、全てのトラフィックは遮断されているため、"ping"、 "ssh"などのサービスさえ使用できず、必要なサービスを開くとサービスが可能です。これはインターネットからアクセスしても、同じローカルからアクセスしても、同じように適用されます。また、さらにセキュリティーグループを追加して規則を加えてインスタンスに適用できます。 

例えば"SSH"という規則に"INGRESS TCP PORT 23"、 "WEB"という規則に"INGRESS TCP PORT 80"という規則があるとすると、1個のインスタンスに"SSH"、 "WEB" 2個のセキュリティーグループを設定すると2つのサービスを全て使用できます。


項目 | 説明
-------- | ---------- 
Direction | Ingressインスタンスに流入する方向を意味します。 Egressはインスタンスに出る方向を意味します。
Ether Type | IPのバージョンを意味します。
IPプロトコル | 特定のプロトコルを指定するか、全体を指定できます。
ポート範囲 | L4プロトコルの場合、ポートの範囲を指定できます。
遠隔 | 遠隔地アドレスまたはIPアドレス範囲です。 Ingressの場合は送信元アドレス、 Egressの場合、宛先アドレスになります。

セキュリティーグループは"Stateful"で動作するため、規則により一度接続したセッションは反対の方向のルールがなくても許容されます。例えば"INGRESS TCP PORT 80"によりインスタンスに向かうTCP 80の最初のパケットが規則により通過したら、インスタンスのTCP 80ポートから転送されるパケットは遮断されないことを意味します。ただしコネクションレス型プロトコルの場合、一定時間転送がなければ、その後は遮断します。
...
ルーティングテーブルリストを指定する場合、詳細画面に要約された情報が表記され、 "Routes"タブを利用して経路を追加できます。
